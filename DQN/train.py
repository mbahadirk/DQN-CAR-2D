...
def main():
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_p:
                    ...
                    start_training(car, rays, score, env, agent)  # Start training
                    env.reset()
                    reset_game()
                    env.reward = 0
        ...
        state = env.get_state()  # Get the current state
        ...
        car.draw(window)
        pygame.display.update()  # Don't forget to update the screen

        print(env.reward / 1000)

        clock.tick(tick_rate)  # Control the FPS
...
def start_training(car, rays, score, env, agent):
    ...
    for e in range(episodes):
        ...
        for time in range(500):  # Maximum number of steps
            # Select a random action (or agent action)
    ...
    agent.replay(batch_size)  # Learn from the replay buffer
